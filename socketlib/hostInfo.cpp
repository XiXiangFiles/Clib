...
void infoHost::setHostmac(char **interface){
	struct ifreq ifr;
...
void infoHost::setHostip( char *interface){
...
infoHost::infoHost(char *interface){
...
infoHost::infoHost(infoHost &host){

}
char* infoHost::getMac(){
	return src_mac;
}
char* infoHost::getIP(){
	return srcIP;
}
char* infoHost::getIP6(){
	return srcIP6;
}
